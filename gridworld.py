...
class Environment:
    ...
    def iterate(self, action=None) -> bool:
        ...
        if action is None:
            ...
            #print("position", self.position, "want get_action", action)
        self.__act(action)
...
def main():
    ...
    for e in range(1, N):
        ...
        for r in range(1, N):
            r_=1.0/float(N)*r
            game = Gridworld(grid)
            agent = agents.QLearningAgent(game, args.discount, r_, e_)
            env = Environment(agent, args.noise, args.living_reward, grid)
            for _ in range(50):
                env.iterate()
            if [agent.get_best_policy(state) for state in [(1,1),(1,2),(1,3),(1,4)]] == [Gridworld.Action.Right, Gridworld.Action.Right, Gridworld.Action.Right, Gridworld.Action.Right]:
                print(e_, r_)
